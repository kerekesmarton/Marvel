...
public protocol EmptyStateDataSource: class {
     ...
     */
    var emptyStateButtonModel: PrimaryActionButton.DataModel? { get }
     ...
     */
    func emptyStateViewAnimation(for view: UIView, animationDuration: TimeInterval, completion: ((Bool) -> Void)?) -> Void
}

extension EmptyStateDataSource where Self: UIViewController {
    /// Default implementation of `emptystateView` returns a `UIEmptyStateView`
    public var emptyStateView: UIView {
        get {
            let emptyStateView = EmptyStateView(frame: view.frame, title: emptyStateTitle)
            ...
            emptyStateView.buttonDataModel = emptyStateButtonModel
...
extension EmptyStateDataSource where Self: UIView {
    /// Default implementation of `emptystateView` returns a `UIEmptyStateView`
    public var emptyStateView: UIView {
        get {
            let emptyStateView = EmptyStateView(frame: frame, title: emptyStateTitle)
            // Call and assign the data source methods
            emptyStateView.image = emptyStateImage
            emptyStateView.imageSize = emptyStateImageSize
            emptyStateView.imageViewTintColor = emptyStateImageViewTintColor
...
extension EmptyStateDataSource where Self: UIResponder {
    ...
    public var emptyStateButton: PrimaryActionButton? { get { return nil } }
    
    
    /// Default implementation of `emptyStateButtonModel`, returns nil
    public var emptyStateButtonModel: PrimaryActionButton.DataModel? { get { return nil } }
    ...
    public func emptyStateViewAnimation(for view: UIView, animationDuration: TimeInterval, completion: ((Bool) -> Void)?) -> Void {
        guard let emptyStateView = view as? EmptyStateView else { return }
        // Set initial alpha
        emptyStateView.imageView.alpha = 0.0
        emptyStateView.titleLabel.alpha = 0.0
        emptyStateView.detailLabel.alpha = 0.0
        emptyStateView.button.alpha = 0.0
        // Set initial scale to 0
        emptyStateView.imageView.transform = CGAffineTransform(scaleX: 0.0, y: 0.0)
        emptyStateView.button.transform = CGAffineTransform(scaleX: 0.0, y: 0.0)
        
        UIView.animateKeyframes(withDuration: animationDuration, delay: 0.0, options: [], animations: {
            
            UIView.addKeyframe(withRelativeStartTime: 0.0, relativeDuration: 1/3, animations: {
                emptyStateView.imageView.alpha = 1.0
                emptyStateView.imageView.transform = .identity
            })
            
            UIView.addKeyframe(withRelativeStartTime: 1/3, relativeDuration: 1/3, animations: {
                emptyStateView.titleLabel.alpha = 1.0
                emptyStateView.detailLabel.alpha = 1.0
            })
            
            UIView.addKeyframe(withRelativeStartTime: 2/3, relativeDuration: 1/3, animations: {
                emptyStateView.button.alpha = 1.0
                emptyStateView.button.transform = .identity
            })
...
extension SecondaryActionButton.DataModel {
    
    func size() -> CGSize? {
        switch self {
        case .normal(text: let text, image: _):
            return text?.size()
        case .translucent(text: let text, image: _):
            return text?.size()
        case .filled(text: let text, image: _):
            return text?.size()
        case .warning(text: let text):
            return text.size()
        }
    }
}
