...
class CharacterListPresenter: CharacterListPresenting {
    ...
    var results: Entities.CharacterDataWrapper? {
        didSet {
            output.reload()
        }
    }
    
    var unwrappedItems: [Entities.Character]? {
    ...
    var emptyStateTitle: NSAttributedString {
        ...
        return output.makeAttributedString(from: [FontCalculable(text: "channel_list_empty_state_title".localised, style: .mention)])
    }
    
    public var emptyStateTitleDetail: NSAttributedString? {
        guard let view = output else { return NSAttributedString() }
        return view.makeAttributedString(from: [FontCalculable(text: "channel_list_empty_state_details".localised, style: .normal)])
    }
    
    public var emptyStateButtonTitle: String? {
        return "channel_list_empty_state_button".localised
    }
